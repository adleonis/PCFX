...
import org.pcfx.adapter.android.config.AdapterBuildConfig
import java.util.concurrent.TimeUnit

class PDVHealthCheckClient(
    ...
    fun getUniqueAppId(): String = AdapterBuildConfig.UNIQUE_APP_ID
    fun getAppName(): String = AdapterBuildConfig.APP_NAME
    fun getAppVersion(): String = AdapterBuildConfig.APP_VERSION

    companion object {
        private const val TAG = "PDVHealthCheckClient"
        private var instance: PDVHealthCheckClient? = null

        fun getInstance(context: Context): PDVHealthCheckClient {
            return instance ?: PDVHealthCheckClient(context).also { instance = it }
        }
    }
}
